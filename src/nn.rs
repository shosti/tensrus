...
use crate::tensor2::Tensor2;
use crate::var::Var;

pub trait Module<T: Numeric> {
    fn parameters(&self) -> Vec<Var<Scalar2<T>>>;
}
...
impl<T: Numeric> Neuron<T> {
        ...
        StandardNormal: Distribution<T>,
    {
        let mut w = Vec::new();
        for _ in 0..nin {
            w.push(Var::new(Scalar2::randn(&mut thread_rng())));
        }
    ...
    pub fn call(&self, x: &[Var<Scalar2<T>>]) -> Var<Scalar2<T>> {
        let wx: Var<Scalar2<T>> = std::iter::zip(self.w.clone(), x)
...
impl<T: Numeric> Layer<T> {
    ...
    pub fn call(&self, x: &[Var<Scalar2<T>>]) -> Vec<Var<Scalar2<T>>> {
        let mut out = Vec::new();
...
impl<T: Numeric> MLP<T> {
    ...
    pub fn call(&self, x: Vec<Var<Scalar2<T>>>) -> Vec<Var<Scalar2<T>>> {
        let mut res = x;
    ...
    pub fn loss(&self, ys: &[Var<Scalar2<T>>], ypred: &[Var<Scalar2<T>>]) -> Var<Scalar2<T>> {
